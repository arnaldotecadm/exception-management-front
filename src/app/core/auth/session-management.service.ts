...
export class SessionManagementService {
  ...
  adicionarAcesso(sessao: SessionManagementInterface) {
    return this.http.post(apiURL + '/add', sessao);
  }

  removerAcessoByUsuario(usuario: string) {
    return this.http.delete(apiURL + '/remove/usuario/' + usuario);
  }

  removerAcessoByUUID(uuid: string) {
    return this.http.delete(apiURL + '/remove/uuid/' + uuid);
  }

  cancelarAcessoByIp(ip: string) {
    return this.http.post(apiURL + '/cancelar-acesso/ip', ip);
  }

  cancelarAcessoByUUID(uuid: string) {
    return this.http.post(apiURL + '/cancelar-acesso/uuid', uuid);
  }
}
